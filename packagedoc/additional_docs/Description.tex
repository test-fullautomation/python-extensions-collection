...
\begin{enumerate}
\item \pfcode{CComparison}\\
   Compare two text files based on regular expressions.\\
   Import: \pfcode{from PythonExtensionsCollection.Comparison.CComparison import CComparison}
\item \pfcode{CFile}\\
   File operations like read, write, copy, move, ...\\
   Import: \pfcode{from PythonExtensionsCollection.File.CFile import CFile}
\item \pfcode{CFolder}\\
   Folder operations like create, delete, copy, ...\\
   Import: \pfcode{from PythonExtensionsCollection.Folder.CFolder import CFolder}
\item \pfcode{CString}\\
   String operations like normalize a path, string filter, format results, ...\\
   Import: \pfcode{from PythonExtensionsCollection.String.CString import CString}
\item \pfcode{CUtils}\\
   Pretty print of Python data types\\
   Import: \pfcode{from PythonExtensionsCollection.Utils.CUtils import CTypePrint}
\end{enumerate}
...
We are inside folder \rlog{GHI} and want to know the path to folder \rlog{ABC}.
...
We are inside folder \rlog{GHI} and want to know the path to folders \rlog{ABC} and \rlog{DEF}.
...
Therefore the folder \rlog{DEF} is the first one who is found. The order of elements in \pcode{listDestPaths} is not synchronized
...
We are inside folder \rlog{GHI} and want to know the path to folder \rlog{DEF} and within this folder we want to know the path to file \rlog{FILE.txt}.
...
\begin{itemize}
   \item The type of input parameter \rlog{oData} is \rlog{dict}
   ...
   \item The fourth element of the list is of type \rlog{tuple}
   \item The tuple contains 2 elements
   \item The current line gives information about the first element of the tuple
   \item The first element of the tuple is of type \rlog{int} and has the value \rlog{33}
\end{itemize}
